...
from .structural_models.SETMbase import StructuralETMBase
from optuna.integration import PyTorchLightningPruningCallback
import matplotlib.pyplot as plt

time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
MODEL_NAME = "StrcuturalETM"
# logger.add(f"{MODEL_NAME}_{time}.log", backtrace=True, diagnose=True)


class StructuralETM(BaseModel):
    """
    ...
    **kwargs : dict
    ...
    def _initialize_model(self):
        ...
        This method initializes the neural base model (`StructuralNeuralBaseModel`) with the given
        ...
        **kwargs
        ...
        self.theta = (
            self.model.model.get_theta(
                torch.tensor(self.dataset.bow),
                torch.tensor(self.dataset.features),
                only_theta=True,
            )
            .detach()
            .cpu()
