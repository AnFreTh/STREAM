    ...
    def _create_vocab_preprocess(self, data, preprocess=5, process_data=False):
        ...
        if self.language == NLTK_STOPWORD_LANGUAGE:
            ...
            for file_num in range(0, len(data)):
                ...
                # seg_result = seg.seg(words)
                # words = [term.word for term in seg_result]
                # segmented_text = [char for word in words for char in word if char.strip()]
                # words = [char for char in segmented_text]
                proc_file = []
        ...
        for k in range(self.ntopics):
            ...
            for i in range(ntopw - 1):
                for j in range(i + 1, ntopw):
                    ...
                    print(w1_dc)
                    print(word_doc_counts.get(w1, set()))
                    print(w2_dc)
                    print(word_doc_counts.get(w2, set()))
                    print(w1w2_dc)
                    print(word_doc_counts.get(w1, set()) & word_doc_counts.get(w2, set()))
                    # Correct eps:
                    pmi_w1w2 = np.log(
