...
from .ctmneg_utils.ctmn import CTMNModel
...
class CTMN(AbstractModel):
    ...
    def train_model(self, dataset, hyperparameters=None, top_words=10, save_dir=None):
        ...
        if self.use_partitions:
            ...
            self.model.fit(x_train, x_valid, verbose=True, save_dir=save_dir)
            ...
            self.model.fit(x_train, None, verbose=True, save_dir=save_dir)
            result = self.model.get_info()
            return result
    ...
    def preprocess(
        ...
        b_train = CTMN.load_bert_data(bert_train_path, train, bert_model)
        ...
        if test is not None and validation is not None:
            ...
            b_test = CTMN.load_bert_data(bert_test_path, test, bert_model)
