...
class CTMNModel:
    ...
    def __init__(
        ...
        assert (
            isinstance(momentum, float) and momentum > 0 and momentum <= 1
    ...
    def _loss(
        ...
        if self.neg_method:
            TL = self.triplet_loss(word_dists, inputs, word_dists_neg)
        else:
            TL = 0
    ...
    def predict(self, dataset):
        ...
        with torch.no_grad():
            for batch_samples in loader:
                ...
                # forward pass
                self.model.zero_grad()
                _, _, _, _, _, _, _, topic_document, word_dists_neg = self.model(
                    X, X_bert
                )
    ...
    def get_topics(self, k=10):
        ...
        if self.num_topics is not None:
            for i in range(self.num_topics):
                ...
                component_words = [
                    self.train_data.idx2token[idx] for idx in idxs.cpu().numpy()
                ]
