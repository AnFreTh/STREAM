...
class CTMNegBase(nn.Module):
    """
    CTMNegBase is a neural network-based topic modeling class that supports both ProdLDA and LDA models.
    ...
    def forward(self, x):
        ...
        theta_neg = self.perturbTheta(theta, self.topic_perturb)
        ...
        return word_dist, mu, logvar, word_dist_neg
    ...
    def compute_loss(self, x):
        ...
        torch.Tensor
            The computed loss.
        """
        word_dist, mu, logvar, word_dist_neg = self.forward(x)
        loss = self.loss_function(x["bow"], word_dist, mu, logvar, word_dist_neg)
        if self.rescale_loss:
            loss *= self.rescale_factor

        return loss

    def loss_function(self, x, word_dist, mu, logvar, word_dist_neg):
        """
        Computes the reconstruction and KL divergence loss.
        ...
        torch.Tensor
            The computed loss.
        """
        recon_loss = -(x * (word_dist + 1e-10).log()).sum(axis=1)
        var = logvar.exp()
        ...
        TL = self.triplet_loss(word_dist, x, word_dist_neg)
        return loss + self.tl_weight * TL
    ...
    def perturb(x):
        ...
        -------
        torch.Tensor
            Perturbed tensor with Gaussian noise.
        """
    ...
    def perturbTopK(x, k):
        """
        Perturb the top K values in the input tensor.
        ...
        x : torch.Tensor
            Input tensor.
        k : int
            Number of top values to perturb.

        Returns
        -------
        torch.Tensor
            Tensor with top K values perturbed.
        """
    ...
    def perturbTheta(x, k):
        """
        Perturb the theta values by calling perturbTopK and normalizing.
        ...
        -------
        torch.Tensor
            Perturbed and normalized theta values.
        """
        x_new = CTMNegBase.perturbTopK(x, k)
