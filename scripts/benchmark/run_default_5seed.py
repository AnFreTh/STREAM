"""
Script 2: Default benchmark with 5-seed stability analysis.

Same default hyperparameters, 5 different random seeds per model per dataset.
This is the main benchmark for the paper — tests initialization sensitivity.
Results saved to S3 under version="v2_default_5seed".
"""
...
def main():
    ...
    # Per-model mean/std summary
    metrics = ["NMI", "Purity"] + [f"{m}@{k}" for m in ["CV","CV_train","NPMI","TD","ISIM","INT","ISH","Emb_Coherence","Emb_TD"] for k in [5,10,15,20]]
    summary = df.groupby(["dataset", "model"])[metrics].agg(["mean", "std"])
    s3_put_csv(summary.reset_index(), f"{S3_PREFIX}/_results/{VERSION}_summary{suffix}.csv")

    logger.info(f"\nDone! {len(all_results)} runs ({len(DATASETS)} datasets x {len(ALL_MODELS)} models x {len(SEEDS)} seeds)")
